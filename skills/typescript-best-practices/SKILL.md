...
> In the original Cursor plugin this skill auto-attached whenever a `.ts` or `.tsx` file was open (`paths: ["**/*.ts", "**/*.tsx"]`). Claude Code has no path-based auto-attach frontmatter, so apply this skill yourself whenever you're reading or editing TypeScript.
