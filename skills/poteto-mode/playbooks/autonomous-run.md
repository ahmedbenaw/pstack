...
2. Pick the wake mechanism using the host's `/loop` command (a built-in on Cursor and on Claude Code, not a pstack skill; on a host without one, arm an equivalent scheduled wake). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
