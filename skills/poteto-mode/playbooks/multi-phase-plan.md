...
**Control skill.** Pick it by surface. Use the project-local verification skill generated by **create-verification-skill** (`/create-verification-skill`), which covers browser, Electron and web UIs as well as CLIs and TUIs. **maintain-verification-skill** keeps it current. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Triage every automated-review and security-reviewer comment per `../references/review-triage.md`.
...
Each live lane runs on its own cloud VM at the PR head. Drive through the project's verification skill (see **Control skill** above).
