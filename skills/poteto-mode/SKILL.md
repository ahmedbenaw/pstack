...
- Before commit → the `deslop` skill from the `cursor-team-kit` plugin (`/deslop`). *Note: `cursor-team-kit` is a separate Cursor plugin and is not vendored in this Claude Code port; if it isn't installed, do the same cleanup in plain words instead.*
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control skill. `cursor-team-kit` publishes `control-cli` (CLIs and TUIs) and `control-ui` (browser / Electron / web UIs). *Note: `cursor-team-kit` is not vendored in this port; without it, drive and verify the surface manually.* For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
