...
> In the original Cursor plugin this skill was a "sticky mode" (`mode: true`) that, once entered, stayed applied across turns until the user opted out, with a runtime reminder nudging the agent to re-apply it: "New task? Playbook match or rigor needed -> apply /poteto-mode. Casual turn or user opts out -> don't." Claude Code has no sticky-mode frontmatter, so treat that as operating guidance: once you start working in this style for a task, keep applying it across turns of that task unless the user opts out, and re-check it at the start of each new task.
...
- Before commit → strip code slop from the diff (remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits; the **subtract-before-you-add** principle skill is the rule, and `/simplify` does it in one pass on Claude Code). Prose, including the PR body and commit messages, gets the **unslop** skill (`/unslop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control skill. Generate one with **create-verification-skill** (`/create-verification-skill`): it writes a project-local skill that drives this repo's own surface, CLI and TUI as well as browser, Electron and web UI. **maintain-verification-skill** keeps it honest as the app changes. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
- Any PR-status request → the **Babysit** playbook (`playbooks/babysit.md`). On a host that ships its own babysit skill whose description matches the same words, this playbook wins. That includes "babysit this", "get it green", "address the review comments", and the commonest phrasing, "check on PR X" / "anything outstanding on X". Never triggered by merely opening a PR. Declare its mode before polling. The playbook's step 1 owns the request-to-mode mapping. Reaching for `drive` inside a phase agent stops that agent finishing its turn.
- Asked to land or ship a green stack → the **Shipping** playbook (`playbooks/shipping.md`). Green is not safe. Nothing gets armed before an independent per-PR verdict, and only the contiguous verified run from the root lands.
- An automated reviewer commented, whether a bot on the PR or a review you ran yourself such as `/code-review` → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/review-triage.md`.
