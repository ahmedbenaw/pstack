...
// down from the checked-out branch through each PR's base ref until a branch has
// no PR (trunk), then reverse so the frontier reads bottom-to-top. This is the
// same ordering `gt log short --stack --reverse` produced, resolved from the
// forge the rest of this port already depends on.
interface GhPrRow {
  readonly number: number;
...
function ghPrRows(repo: string): readonly GhPrRow[] {
  let raw: string;
  try {
    raw = execFileSync(
      "gh",
  ...
  try {
    ...
    throw new UserError(
      `gh pr list did not return JSON: ${JSON.stringify(raw.slice(0, 120))}`
    );
  }
...
function currentBranch(repo: string): string {
  ...
  if (branch.length === 0 || branch === "HEAD") {
    throw new UserError(
      "the repository is not on a named branch, so there is no stack to resolve"
    );
  }
  return branch;
}
...
function validateFrontierPin({
  ...
  const drift: string[] = [];
  if (missing.length > 0) {
    drift.push(`missing from the forge: ${missing.join(",")}`);
  }
  if (extra.length > 0) {
    drift.push(`extra on the forge: ${extra.join(",")}`);
  }
