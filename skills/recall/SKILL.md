...
Transcripts live under a per-workspace directory whose name is the workspace path with its separators flattened. On Claude Code and Cowork that is `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` replaces every non-alphanumeric character with "-" (so `/Users/you/proj` becomes `-Users-you-proj`). On Cursor it is `~/.cursor/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl`, where `<slug>` drops the leading slash and turns each "/" into "-" (`Users-you-proj`). Use whichever store exists; when both do, prefer the host you are running on. Every line is one chat message.
