...
This skill orchestrates three others: an inline mining pass (see step 1), skill authoring (`playbooks/authoring-a-skill.md`, or Cursor's built-in `create-skill` where it exists), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Author the skill per `playbooks/authoring-a-skill.md` (on Cursor, the built-in `create-skill` does it in one pass). Placement:
...
Apply the **unslop** skill to every line.
...
- User wants a task-specific skill (not working conventions): author it directly per `playbooks/authoring-a-skill.md`, no mining required.
