...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across `~/.claude/projects/*/` (or `~/.cursor/projects/*/` on Cursor). That crosses workspace boundaries and reads private chats from unrelated projects.
