...
- `/loop` is the host's built-in wake mechanism, not a pstack skill; Cursor and Claude Code both ship one, and on a host without one you arm an equivalent scheduled wake. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
