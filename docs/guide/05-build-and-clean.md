...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) strips code slop from the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. Upstream pstack delegates the code half to `/deslop` from Cursor's `cursor-team-kit` plugin, which this port cannot install. Ask for the outcome instead: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits. On Claude Code, `/simplify` does exactly this in one pass; [`/principle-subtract-before-you-add`](../../skills/principle-subtract-before-you-add/SKILL.md) is the rule behind it.
...
The division of labor is worth keeping straight. The code-slop strip cleans slop out of the code, `/unslop` cleans it out of prose, and `/no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, say `strip the slop` before you commit, not after review calls it out.
